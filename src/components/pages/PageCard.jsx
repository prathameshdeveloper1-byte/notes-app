    ...
    <>
      <motion.div
        ...
        onClick={() => {
          ...
          const bookId = page.bookId || activeBookId;
          router.push(`/books/${bookId}/pages/${page.id}`);
        }}
      >
        {/* Color bar */}
        {!page.color && (
        ...
        <div className="p-4">
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <div className="flex items-center gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={handleStar} className="p-1 hover:bg-black/5 rounded-lg" title="Star note">
                <Star size={13} className={cn(page.starred ? 'fill-amber-400 text-amber-400' : 'text-ink-300')} />
              </button>
              <button onClick={(e) => { e.stopPropagation(); setMenuOpen(v => !v); }} className="p-1 hover:bg-black/5 rounded-lg" title="More options">
      ...
      <ConfirmDeleteModal
        ...
        onConfirm={() => deletePage(page.id, activeBookId, activeFolderId)}
        title="Delete Page"
        itemName={page.title || 'Untitled Page'}
        description="Are you sure you want to delete this page? This note and all its contents will be permanently removed."
        itemType="page"
      />
    </>
  );
